...
def open_text_input():
    def on_submit():
        ...
        def ping_and_process():
            if not ping_server():
                ...
                return
            root.after(0, lambda: result_label.config(text="Serveur disponible. Envoi en cours...", bootstyle="info"))
            def worker():
    ...
    def loading_animation():
        ...
        while not loading_event.is_set():
            result_label.config(text=f"Traitement en cours... {spinner[i % len(spinner)]}", bootstyle="warning")
            i += 1
            time.sleep(0.15)
        result_label.config(text="")  # Nettoie le texte après l'arrêt de l'animation
